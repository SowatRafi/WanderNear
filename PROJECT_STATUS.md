...
Known gaps, deliberately deferred: a download is tied to the Preferences screen (same limit as the AI-model download); downloaded packs have no Wikipedia summaries (v1 skips enrichment), so "Worth visiting nearby" is thinner there than in bundled Melbourne — and Travel Mode "story" alerts only fire in the bundled city until enrichment lands. (**Deleting a city is now supported** — a trash button per pack in the Cities card, including the built-in Melbourne, which is restorable via a hidden marker; you always keep at least one city.)
