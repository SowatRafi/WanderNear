...
- **M6.5** Essentials near you (next) — hospital / fuel / parking added to the pipeline (named-only → grounded) + a "daily needs near you" home card (Directions/Call) + a reverse-geocoded suburb label. Grounded from OSM/Wikipedia, NOT web-scraped (offline + never-hallucinate). Motivated by "why does it say Melbourne when I'm in Werribee?".
